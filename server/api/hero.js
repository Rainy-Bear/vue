...
router.get("/getOneTypeList", function(req, res, next) {
  ...
  var sql =
    "select hero.heroId,hero.name,hero.title,hero.imgUrl,hero.detailImg from hero,hero_mat_type where hero_mat_type.heroTypeId = ? and hero_mat_type.heroId = hero.heroId";
...
router.get("/getHeroType", function(req, res, next) {
  ...
  var sql =
    "select herotype.herotypeId,herotype.heroTypeName from hero_mat_type,herotype where heroId = ? and hero_mat_type.heroTypeId = heroType.heroTypeId";
  var params = [condition.heroId];
  connection.query(sql, params, function(err, result) {
    res.send(result);
  });
