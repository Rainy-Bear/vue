...
router.get('/getEquip', function(req, res, next) {
  ...
  var startSql =
    'select goods.imgUrl,goods.goodId from startequip,goods where heroId = ? and mapId = ? and startequip.goodId = goods.goodId';
  var kernelSql =
    'select goods.imgUrl,goods.goodId from kernelequip,goods where heroId = ? and mapId = ? and kernelequip.goodId = goods.goodId';
  var fitSql =
    'select goods.imgUrl,goods.goodId from fitequip,goods where heroId = ? and mapId = ? and fitequip.goodId = goods.goodId';
  var defenderSql =
    'select goods.imgUrl,goods.goodId from defenderequip,goods where heroId = ? and mapId = ? and defenderequip.goodId = goods.goodId';
...
router.get('/getOneGoodTypeList', function(req, res, next) {
  ...
  var sql =
    'select goods.name,goods.goodId,goods.imgUrl from goods,good_mat_type where good_mat_type.goodTypeId = ? and good_mat_type.goodId = goods.goodId';
...
router.get('/getGoodType', function(req, res, next) {
  ...
  var sql =
    'select distinct goodTypeName from goodtype,good_mat_type where goodId = ? and goodType.goodTypeId = good_mat_type.goodTypeId';
  var params = [condition.goodId];
  connection.query(sql, params, function(err, result) {
    res.send(result);
  });
