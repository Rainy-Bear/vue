<template>
  <div class="goods">
    ...
    <div class="oneGoodTypeList">
      <router-link
        v-for="(item,index) in oneGoodTypeList"
        :key="item.goodId"
        :to="{name:'goodDetail',params:{goodId:item.goodId}}"
        class="oneGood"
...
export default {
  ...
  methods: {
    goodTypeClick: function(index) {
      ...
      this.$http
        .get(
          "/api/goods/getOneGoodTypeList",
          { params: { goodTypeId: index + 1 } },
          { emulateJSON: true }
        )
        .then(result => {
  ...
  mounted: function() {
    //获取所有物品类型
    this.$http
      .get("/api/goods/getAllGoodType", { emulateJSON: true })
      .then(result => {
    ...
    this.$http
      .get(
        "/api/goods/getOneGoodTypeList",
        { params: { goodTypeId: this.indexs + 1 } },
        { emulateJSON: true }
      )
      .then(result => {
        this.oneGoodTypeList = result.data;
