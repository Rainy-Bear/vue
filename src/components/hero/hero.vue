  ...
  <div class="hero">
    <div class="heroType">
      ...
      <div class="heroTypeCon">
        <router-link class="typeList" v-for="(item, index) in typeListArr" :key="item.heroTypeId"
                     :to="{name:'heroList',params:{heroTypeId: item.heroTypeId}}"
                     :class="{marRight:(index + 1) % 3 !== 0,marTop:index > 2}">
          <img :src="item.heroTypeImg" alt="">
          <span :class="{bgOne: index===0,bgTwo: index===1,bgThree: index===2,bgFour: index===3,bgFive: index===4,bgSix: index===5}"></span>
...
<script>
  export default {
    ...
    mounted: function () {
      this.$nextTick(() => {
        this.$http.get('/api/heroType/heroType').then((result) => {
