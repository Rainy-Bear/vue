<template>
  <div class="useSkill">
    ...
    <div class="useSkillIntro">
      <div class="whoUse">
        <span
          v-for="(item,index) in whoUse"
          @click="whoUseClick(index)"
          :class="{active: index === indexs}">{{item}}</span>
      </div>
      <div class="useIntro">
        <span
          v-for="(item,index) in myselfUseIntro"
          :class="{marTop: index > 0}"
          v-if="indexs === 0">{{item}}</span>
        <span
          v-for="(item,index) in enemyUseIntro"
          :class="{marTop: index > 0}"
          v-if="indexs === 1">{{item}}</span>
      </div>
    </div>
  </div>
</template>
